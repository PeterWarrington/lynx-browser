#ifndef CFG_DEFS_H
#define CFG_DEFS_H 1

static const struct {
	const char *name;
	const char *value;
} config_cache[] = {
	{ "",	"" },
	{ "SYSTEM_MAIL",	"/usr/sbin/sendmail" },
	{ "ar_flags",	"-curv" },
	{ "baddef_remove",	"no" },
	{ "bool_defs",	"yes" },
	{ "build",	"aarch64-apple-darwin25.5.0" },
	{ "build_alias",	"aarch64-apple-darwin25.5.0" },
	{ "c11_noreturn",	"no," },
	{ "c_compiler_gnu",	"yes" },
	{ "c_inline",	"inline" },
	{ "chtype_decl",	"yes" },
	{ "chtype_type",	"scalar" },
	{ "dcl_errno",	"yes" },
	{ "define_sigwinch",	"yes" },
	{ "env_CC_set",	"" },
	{ "env_CC_value",	"" },
	{ "env_CFLAGS_set",	"" },
	{ "env_CFLAGS_value",	"" },
	{ "env_CPPFLAGS_set",	"set" },
	{ "env_CPPFLAGS_value",	"-I/opt/homebrew/opt/s-lang/include" },
	{ "env_CPP_set",	"" },
	{ "env_CPP_value",	"" },
	{ "env_LDFLAGS_set",	"set" },
	{ "env_LDFLAGS_value",	"-L/opt/homebrew/opt/s-lang/lib" },
	{ "env_build_alias_set",	"" },
	{ "env_build_alias_value",	"" },
	{ "env_host_alias_set",	"" },
	{ "env_host_alias_value",	"" },
	{ "env_target_alias_set",	"" },
	{ "env_target_alias_value",	"" },
	{ "find_linkage_brotlidec",	"no" },
	{ "find_linkage_bz2",	"yes" },
	{ "find_linkage_iconv",	"yes" },
	{ "find_linkage_idn2",	"no" },
	{ "find_linkage_idn",	"no" },
	{ "find_linkage_zstd",	"no" },
	{ "fionbio",	"ioctl" },
	{ "func_SLtt_get_screen_size",	"no" },
	{ "func_acos",	"yes" },
	{ "func_alloca_works",	"yes" },
	{ "func_arc4random",	"yes" },
	{ "func_atoll",	"yes" },
	{ "func_ctermid",	"yes" },
	{ "func_cuserid",	"no" },
	{ "func_decl_getgrgid",	"yes" },
	{ "func_decl_getgrnam",	"yes" },
	{ "func_decl_sleep",	"yes" },
	{ "func_decl_strstr",	"yes" },
	{ "func_fork",	"yes" },
	{ "func_fork_works",	"yes" },
	{ "func_fseeko",	"yes" },
	{ "func_ftime",	"yes" },
	{ "func_getcwd",	"yes" },
	{ "func_getgroups",	"yes" },
	{ "func_gethostbyname",	"yes" },
	{ "func_gethostname",	"yes" },
	{ "func_getpwuid",	"yes" },
	{ "func_gettimeofday",	"yes" },
	{ "func_getuid",	"yes" },
	{ "func_iconv",	"yes" },
	{ "func_inet_ntoa",	"yes" },
	{ "func_lstat",	"yes" },
	{ "func_mkdtemp",	"yes" },
	{ "func_mktime",	"yes" },
	{ "func_popen",	"yes" },
	{ "func_putenv",	"yes" },
	{ "func_readdir",	"yes" },
	{ "func_setuid",	"yes" },
	{ "func_sigaction",	"yes" },
	{ "func_sleep",	"yes" },
	{ "func_socket",	"yes" },
	{ "func_strcasecmp",	"yes" },
	{ "func_strerror",	"yes" },
	{ "func_strstr",	"yes" },
	{ "func_truncate",	"yes" },
	{ "func_ttyname",	"yes" },
	{ "func_unsetenv",	"yes" },
	{ "func_usleep",	"yes" },
	{ "func_vasprintf",	"yes" },
	{ "func_vfork",	"yes" },
	{ "func_vfork_works",	"yes" },
	{ "func_waitpid",	"yes" },
	{ "func_wcwidth",	"yes" },
	{ "have_decl_exit",	"yes" },
	{ "have_errno",	"yes" },
	{ "have_h_errno",	"yes" },
	{ "have_inet_aton",	"yes" },
	{ "have_lib_slang",	"yes" },
	{ "have_utmp",	"utmpx" },
	{ "have_utmp_ut_host",	"yes" },
	{ "have_utmp_ut_name",	"ut_user" },
	{ "have_utmp_ut_session",	"no" },
	{ "have_utmp_ut_syslen",	"no" },
	{ "have_utmp_ut_xstatus",	"no" },
	{ "have_utmp_ut_xtime",	"define" },
	{ "header_arpa_inet_h",	"yes" },
	{ "header_dirent_dirent_h",	"yes" },
	{ "header_fcntl_h",	"yes" },
	{ "header_intl",	"" },
	{ "header_inttypes_h",	"yes" },
	{ "header_lastlog_h",	"no" },
	{ "header_libgt",	"" },
	{ "header_limits_h",	"yes" },
	{ "header_memory_h",	"yes" },
	{ "header_path_brotlidec",	"/opt/homebrew/opt/s-lang/include" },
	{ "header_path_bz2",	"/usr/include" },
	{ "header_path_iconv",	"/usr/include" },
	{ "header_path_idn2",	"/opt/homebrew/opt/s-lang/include" },
	{ "header_path_idn",	"/opt/homebrew/opt/s-lang/include" },
	{ "header_path_zstd",	"/opt/homebrew/opt/s-lang/include" },
	{ "header_paths_h",	"yes" },
	{ "header_pwd_h",	"yes" },
	{ "header_stdc",	"yes" },
	{ "header_stdint_h",	"yes" },
	{ "header_stdlib_h",	"yes" },
	{ "header_string_h",	"yes" },
	{ "header_strings_h",	"yes" },
	{ "header_sys_fcntl_h",	"yes" },
	{ "header_sys_filio_h",	"yes" },
	{ "header_sys_ioctl_h",	"yes" },
	{ "header_sys_param_h",	"yes" },
	{ "header_sys_stat_h",	"yes" },
	{ "header_sys_time_h",	"yes" },
	{ "header_sys_timeb_h",	"yes" },
	{ "header_sys_types_h",	"yes" },
	{ "header_sys_wait_h",	"yes" },
	{ "header_syslog_h",	"yes" },
	{ "header_termio_h",	"no" },
	{ "header_termios_h",	"yes" },
	{ "header_time",	"yes" },
	{ "header_unistd_h",	"yes" },
	{ "header_vfork_h",	"no" },
	{ "header_wchar_h",	"yes" },
	{ "host",	"aarch64-apple-darwin25.5.0" },
	{ "host_alias",	"aarch64-apple-darwin25.5.0" },
	{ "langinfo_codeset",	"yes" },
	{ "lib_dir_opendir",	"no" },
	{ "lib_inet_inet_ntoa",	"no" },
	{ "library_file_bz2",	"-lbz2" },
	{ "library_file_iconv",	"-liconv" },
	{ "library_path_brotlidec",	"" },
	{ "library_path_bz2",	"/usr/lib" },
	{ "library_path_iconv",	"/usr/lib" },
	{ "library_path_idn2",	"" },
	{ "library_path_idn",	"" },
	{ "library_path_zstd",	"" },
	{ "locale",	"yes" },
	{ "make_PHONY",	"yes" },
	{ "mixedcase",	"no" },
	{ "netlibs",	"" },
	{ "ngroups",	"yes" },
	{ "objext",	"o" },
	{ "path_BROTLI",	"/opt/homebrew/bin/brotli" },
	{ "path_BZIP2",	"/usr/bin/bzip2" },
	{ "path_COMPRESS",	"/usr/bin/compress" },
	{ "path_EGREP",	"'grep -E'" },
	{ "path_FGREP",	"'grep -F'" },
	{ "path_GROFF_PATH",	"no" },
	{ "path_GZIP",	"/usr/bin/gzip" },
	{ "path_INSTALL",	"'/usr/bin/install -c'" },
	{ "path_MSGINIT",	"/opt/homebrew/bin/msginit" },
	{ "path_MV",	"/bin/mv" },
	{ "path_NROFF_PATH",	"/usr/bin/mandoc" },
	{ "path_RM",	"/bin/rm" },
	{ "path_TAR",	"/usr/bin/tar" },
	{ "path_TBL_PATH",	"cat" },
	{ "path_UNCOMPRESS",	"/usr/bin/gunzip" },
	{ "path_UNZIP",	"/usr/bin/unzip" },
	{ "path_UUDECODE",	"/usr/bin/uudecode" },
	{ "path_ZCAT",	"/usr/bin/zcat" },
	{ "path_ZIP",	"/usr/bin/zip" },
	{ "path_ZSTD",	"/opt/homebrew/bin/zstd" },
	{ "path_ac_pt_PKG_CONFIG",	"none" },
	{ "path_ac_pt_WINDRES",	"none" },
	{ "path_cf_man2html",	"no" },
	{ "path_install",	"'/usr/bin/install -c'" },
	{ "path_lastlog",	"no" },
	{ "posix_visible",	"no" },
	{ "prog_CPP",	"'gcc -E'" },
	{ "prog_CTAGS",	"ctags" },
	{ "prog_GREP",	"grep" },
	{ "prog_MAKE_LOWER_TAGS",	"yes" },
	{ "prog_YACC",	"'bison -y'" },
	{ "prog_ac_ct_AR",	"ar" },
	{ "prog_ac_ct_CC",	"gcc" },
	{ "prog_ac_ct_RANLIB",	"ranlib" },
	{ "prog_cc_g",	"yes" },
	{ "prog_cc_stdc",	"" },
	{ "prog_cpp_comments",	"yes" },
	{ "prog_make_make_set",	"yes" },
	{ "proto_iconv_arg1",	"'/* nothing */'" },
	{ "proto_iconv_const",	"no" },
	{ "pw_gecos",	"yes" },
	{ "rand_max",	"0xFFFFFFFFUL" },
	{ "screen",	"slang" },
	{ "set_errno",	"yes" },
	{ "sizeof_int",	"4" },
	{ "sizeof_long",	"8" },
	{ "sizeof_off_t",	"8" },
	{ "sizeof_size_t",	"8" },
	{ "sizeof_struct_sockaddr",	"16" },
	{ "sizeof_struct_sockaddr_in",	"16" },
	{ "sizeof_time_t",	"8" },
	{ "slang_header",	"predefined" },
	{ "slang_unix",	"no" },
	{ "slang_utf8",	"no" },
	{ "sockaddr_len",	"yes" },
	{ "socks_lib",	"no" },
	{ "srand_func",	"arc4random_stir/arc4random" },
	{ "struct_dirent64",	"no" },
	{ "sys_file_offset_bits",	"no" },
	{ "sys_large_files",	"no" },
	{ "sys_largefile_CC",	"no" },
	{ "sys_largefile_source",	"no" },
	{ "system_mail_flags",	"'-t -oi'" },
	{ "system_name",	"darwin25.5.0" },
	{ "sysv_utmp",	"yes" },
	{ "target",	"aarch64-apple-darwin25.5.0" },
	{ "target_alias",	"aarch64-apple-darwin25.5.0" },
	{ "termio_and_curses",	"no" },
	{ "termio_and_termios",	"yes" },
	{ "termlib",	"terminfo" },
	{ "tm_gmtoff",	"yes" },
	{ "type_SLsmg_Color_Type",	"yes" },
	{ "type_SLtt_Char_Type",	"yes" },
	{ "type_getgroups",	"gid_t" },
	{ "type_int",	"yes" },
	{ "type_intptr_t",	"yes" },
	{ "type_long",	"yes" },
	{ "type_long_long",	"yes" },
	{ "type_mode_t",	"yes" },
	{ "type_off_t",	"yes" },
	{ "type_pid_t",	"yes" },
	{ "type_size_t",	"yes" },
	{ "type_socklen_t",	"yes" },
	{ "type_ssize_t",	"yes" },
	{ "type_struct_sockaddr",	"yes" },
	{ "type_struct_sockaddr_in",	"yes" },
	{ "type_time_t",	"yes" },
	{ "type_uid_t",	"yes" },
	{ "type_unionwait",	"no" },
	{ "use_libgnutls",	"no" },
	{ "use_libsocks5",	"no" },
	{ "use_libsocks",	"no" },
	{ "use_libssl",	"no" },
	{ "working_alloca_h",	"yes" },
};

static const struct {
	const char *name;
	const char *value;
} config_defines[] = {
	{ "BROTLI_PATH",	"\"/opt/homebrew/bin/brotli\"" },
	{ "BZIP2_PATH",	"\"/usr/bin/bzip2\"" },
	{ "CAN_SET_ERRNO",	"1" },
	{ "CJK_EX",	"1" },
	{ "COMPRESS_PATH",	"\"/usr/bin/compress\"" },
	{ "DEFAULT_LOCALE_CHARSET",	"FALSE" },
	{ "DIRED_SUPPORT",	"1" },
	{ "DISP_PARTIAL",	"1" },
	{ "EXP_CHINESEUTF8_SUPPORT",	"1" },
	{ "GETGROUPS_T",	"gid_t" },
	{ "GZIP_PATH",	"\"/usr/bin/gzip\"" },
	{ "HAVE_ALLOCA",	"1" },
	{ "HAVE_ALLOCA_H",	"1" },
	{ "HAVE_ARPA_INET_H",	"1" },
	{ "HAVE_ATOLL",	"1" },
	{ "HAVE_CTERMID",	"1" },
	{ "HAVE_DIRENT_H",	"1" },
	{ "HAVE_FCNTL_H",	"1" },
	{ "HAVE_FTIME",	"1" },
	{ "HAVE_GETCWD",	"1" },
	{ "HAVE_GETGROUPS",	"1" },
	{ "HAVE_GETPWUID",	"1" },
	{ "HAVE_GETTIMEOFDAY",	"1" },
	{ "HAVE_GETUID",	"1" },
	{ "HAVE_H_ERRNO",	"1" },
	{ "HAVE_ICONV",	"1" },
	{ "HAVE_INET_ATON",	"1" },
	{ "HAVE_INTTYPES_H",	"1" },
	{ "HAVE_LANGINFO_CODESET",	"1" },
	{ "HAVE_LIMITS_H",	"1" },
	{ "HAVE_LONG_LONG",	"1" },
	{ "HAVE_LSTAT",	"1" },
	{ "HAVE_LYHELP_H",	"1" },
	{ "HAVE_MEMORY_H",	"1" },
	{ "HAVE_MKDTEMP",	"1" },
	{ "HAVE_MKTIME",	"1" },
	{ "HAVE_POPEN",	"1" },
	{ "HAVE_PUTENV",	"1" },
	{ "HAVE_PWD_H",	"1" },
	{ "HAVE_READDIR",	"1" },
	{ "HAVE_SETUID",	"1" },
	{ "HAVE_SIGACTION",	"1" },
	{ "HAVE_SLEEP",	"1" },
	{ "HAVE_STDINT_H",	"1" },
	{ "HAVE_STDLIB_H",	"1" },
	{ "HAVE_STRERROR",	"1" },
	{ "HAVE_STRINGS_H",	"1" },
	{ "HAVE_STRING_H",	"1" },
	{ "HAVE_SYSLOG_H",	"1" },
	{ "HAVE_SYS_FCNTL_H",	"1" },
	{ "HAVE_SYS_FILIO_H",	"1" },
	{ "HAVE_SYS_IOCTL_H",	"1" },
	{ "HAVE_SYS_PARAM_H",	"1" },
	{ "HAVE_SYS_STAT_H",	"1" },
	{ "HAVE_SYS_TIMEB_H",	"1" },
	{ "HAVE_SYS_TIME_H",	"1" },
	{ "HAVE_SYS_TYPES_H",	"1" },
	{ "HAVE_SYS_WAIT_H",	"1" },
	{ "HAVE_TERMIOS_H",	"1" },
	{ "HAVE_TRUNCATE",	"1" },
	{ "HAVE_TTYNAME",	"1" },
	{ "HAVE_TYPE_CHTYPE",	"1" },
	{ "HAVE_UNISTD_H",	"1" },
	{ "HAVE_UNSETENV",	"1" },
	{ "HAVE_USLEEP",	"1" },
	{ "HAVE_UTMP",	"1" },
	{ "HAVE_UTMP_UT_HOST",	"1" },
	{ "HAVE_UTMP_UT_XTIME",	"1" },
	{ "HAVE_VASPRINTF",	"1" },
	{ "HAVE_WAITPID",	"1" },
	{ "HAVE_WCHAR_H",	"1" },
	{ "HAVE_WCWIDTH",	"1" },
	{ "HOMEPAGE_URL",	"\"https://lynx.invisible-island.net/\"" },
	{ "ICONV_CONST",	"" },
	{ "INSTALL_ARGS",	"\"-c\"" },
	{ "INSTALL_PATH",	"\"/usr/bin/install\"" },
	{ "LOCALE",	"1" },
	{ "LONG_LIST",	"1" },
	{ "LYNX_CFG_FILE",	"\"/usr/local/etc/lynx.cfg\"" },
	{ "LYNX_CFG_H",	"1" },
	{ "LYNX_CFG_PATH",	"\"/usr/local/etc\"" },
	{ "LYNX_RAND_MAX",	"0xFFFFFFFFUL" },
	{ "MIME_LIBDIR",	"\"/etc/\"" },
	{ "MV_PATH",	"\"/bin/mv\"" },
	{ "OK_GZIP",	"1" },
	{ "OK_OVERRIDE",	"1" },
	{ "OK_PERMIT",	"1" },
	{ "OK_TAR",	"1" },
	{ "OK_UUDECODE",	"1" },
	{ "OK_ZIP",	"1" },
	{ "RM_PATH",	"\"/bin/rm\"" },
	{ "SIZEOF_INT",	"4" },
	{ "SIZEOF_LONG",	"8" },
	{ "SIZEOF_OFF_T",	"8" },
	{ "SIZEOF_SIZE_T",	"8" },
	{ "SIZEOF_TIME_T",	"8" },
	{ "SOCKADDR_LEN_INET",	"1" },
	{ "STDC_HEADERS",	"1" },
	{ "SYSTEM_MAIL",	"\"/usr/sbin/sendmail\"" },
	{ "SYSTEM_MAIL_FLAGS",	"\"-t -oi\"" },
	{ "SYSTEM_NAME",	"\"darwin25.5.0\"" },
	{ "TAR_DOWN_OPTIONS",	"\"-xf\"" },
	{ "TAR_FILE_OPTIONS",	"\"\"" },
	{ "TAR_PATH",	"\"/usr/bin/tar\"" },
	{ "TAR_PIPE_OPTIONS",	"\"-\"" },
	{ "TAR_UP_OPTIONS",	"\"-cf\"" },
	{ "TIME_WITH_SYS_TIME",	"1" },
	{ "UNCOMPRESS_PATH",	"\"/usr/bin/gunzip\"" },
	{ "UNDERLINE_LINKS",	"0" },
	{ "UNIX",	"1" },
	{ "UNZIP_PATH",	"\"/usr/bin/unzip\"" },
	{ "USE_ADDRLIST_PAGE",	"1" },
	{ "USE_ALT_BINDINGS",	"1" },
	{ "USE_ASCII_CTYPES",	"1" },
	{ "USE_BZLIB",	"1" },
	{ "USE_CACHEJAR",	"1" },
	{ "USE_FILE_UPLOAD",	"1" },
	{ "USE_JAPANESEUTF8_SUPPORT",	"1" },
	{ "USE_JUSTIFY_ELTS",	"1" },
	{ "USE_LOCALE_CHARSET",	"1" },
	{ "USE_PERSISTENT_COOKIES",	"1" },
	{ "USE_PRETTYSRC",	"1" },
	{ "USE_PROGRESSBAR",	"1" },
	{ "USE_READPROGRESS",	"1" },
	{ "USE_SESSIONS",	"1" },
	{ "USE_SLANG",	"1" },
	{ "USE_SOURCE_CACHE",	"1" },
	{ "USE_SYSV_UTMP",	"1" },
	{ "UTMPX_FOR_UTMP",	"1" },
	{ "UUDECODE_PATH",	"\"/usr/bin/uudecode\"" },
	{ "ZCAT_PATH",	"\"/usr/bin/zcat\"" },
	{ "ZIP_PATH",	"\"/usr/bin/zip\"" },
	{ "ZSTD_PATH",	"\"/opt/homebrew/bin/zstd\"" },
	{ "lstat",	"stat" },
	{ "lynx_rand",	"arc4random" },
	{ "lynx_srand",	"(void)" },
	{ "ut_name",	"ut_user" },
	{ "ut_xtime",	"ut_tv.tv_sec" },
};

#endif /* CFG_DEFS_H */
