...
#ifdef __3DS__
extern void lynx3ds_trigger_swkbd(const char *prefix);	/* source/slang_shim.c */
#endif
...
#ifdef __3DS__
    /* No physical keyboard: pop up the software keyboard automatically as
     * soon as this field is actually being edited (not just redrawn),
     * instead of requiring a separate button press. Feeds typed
     * characters into the same input queue the loop below reads from, so
     * it's indistinguishable from a real keyboard having typed them. */
    lynx3ds_trigger_swkbd(NULL);
#endif
#ifdef FEPCTRL
    fep_on();
#endif
